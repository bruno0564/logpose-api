...
- **pytest** + **httpx** (357 integration tests)
...
| Habits | `/habits/categories` `/habits/` `/habits/logs` | Habits grouped by colour category, with per-weekday scheduling, optional reminder time and daily completion logs |
...
357 integration tests organised by module. Each test uses an isolated in-memory SQLite database (the `client` fixture in `conftest.py`). They cover equivalence partitions, boundary values and full end-to-end scenarios (complete CRUD flow).
